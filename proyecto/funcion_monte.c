...
void calculo_monte(int N,  int x[], float y[], float o[]){ //Esta función tiene elementos de entrada pero no de salida
	...
	float *m0, *b0, pend, ord, mn, bn, Pmn, mmin, mmax, bmin, bmax, num, sigmam, sigmab;//Hacemos declaración de variables de tipo decimal
	int i, L; //L es las veces que quiere el usuario que se corra el programa para calcular m y b que mejor se ajustan a los datos experimentales
	...
	fp3=fopen("parametros.txt","r"); //Se abre el archivo de lectura que contiene los parámetros
		fscanf(fp3, "%f %f %f %f", &pend, &ord, &sigmam, &sigmab); //SE obtienen datos como "m" y "b" máximas, mínimas e iniciales, así cómo la cantidad de dispersión que se quiere que el archivo dé a los valores
	...
	m0=&pend; //Se le asigna un valor al apuntador de m0 y b0
	b0=&ord;

	fp2=fopen("Resultados.txt","w"); //Se abre el archivo de escritura con el nombre de "Resultados"	
